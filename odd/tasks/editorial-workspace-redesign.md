...
- [x] **UX1** Establish editorial design tokens and shared primitives. Route: delegated; writer trigger (2+ non-trivial files) and preparation trigger. Checks: lint, typecheck, tests, webpack build, browser style spot check passed; final Turbopack build passed outside sandbox.
- [x] **UX2** Recompose the workspace with a clear header, audience navigation, action-oriented overview, and grouped/secondary reference material. Route: delegated; writer trigger. Checks: lint, typecheck, tests, browser desktop/mobile and keyboard pass; final production build passed outside sandbox.
...
- The owner supplied `git@github.com:alejox/market-sell.git`; `origin` was configured and `git push -u origin main` created remote `main` at `e958b9e` with upstream tracking. Local `main` and the feature branch remain aligned without a merge or checkout. The final delivery record is a documentation-only fast-forward; user-owned uncommitted worktree state remains untouched.
